...
template <class matter_t, class background_t> class ComplexScalarCirculation
{
  ...
  public:
    ...
    template <class data_t> void compute(Cell<data_t> current_cell) const
    {
        ...
        data_t v_x = 1.0/( pow(vars.phi_Re,2)+ pow(vars.phi_Im,2)) * (vars.phi_Im*d1.phi_Re[0] - vars.phi_Re*d1.phi_Im[0]);
        data_t v_y = 1.0/( pow(vars.phi_Re,2)+ pow(vars.phi_Im,2)) * (vars.phi_Im*d1.phi_Re[1] - vars.phi_Re*d1.phi_Im[1]);
