...
void ProcaFieldLevel::prePlotLevel()
{
    ...
    ProcaField proca_field(potential, m_p.proca_damping);
    KerrSchildFixedBG kerr_bh(m_p.bg_params, m_dx);
    FixedBGProcaConstraint<Potential, KerrSchildFixedBG> constraint(
        kerr_bh, m_dx, m_p.potential_params.mass,
        m_p.potential_params.self_interaction, potential);
    XSquared set_xsquared(m_p.potential_params, m_p.bg_params, m_p.center,
                          m_dx);
    BoxLoops::loop(make_compute_pack(constraint, set_xsquared), m_state_new,
                   m_state_diagnostics, SKIP_GHOST_CELLS);
}
...
{
    int max_level = m_p.max_level - 1;
    if (m_time > 2000.0)
    {
        max_level = m_p.max_level;
    }
    if (m_level < max_level)
    {
        // if on an active level do usual tagging
        const double radius_bh = m_p.r_min;
        BoxLoops::loop(FixedGridsTaggingCriterionBH(m_dx, m_level, max_level,
                                                    m_p.L, m_p.center,
                                                    radius_bh),
    ...
    {
        // don't tag anything
        BoxLoops::loop(SetValue(0.0), current_state, tagging_criterion);
    }
}
