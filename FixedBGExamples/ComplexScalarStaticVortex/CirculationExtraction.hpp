...
#ifndef CIRCULATIONEXTRACTION_HPP_
#define CIRCULATIONEXTRACTION_HPP_
...
class CirculationExtraction
{
  ...
  public:
    //! The constructor
    CirculationExtraction(int a_comp, int a_num_points, double a_L,
    ...
    //! Destructor
    ~CirculationExtraction() {}

    //! Execute the query
    void execute_query(AMRInterpolator<Lagrange<4>> *a_interpolator,
                       std::string a_file_prefix) const
    {
        CH_TIME("CirculationExtraction::execute_query");
        ...
        {
            interp_x[idx] =
                m_center[0] + ( 0.001* m_L * cos(2.0*3.1416* double(idx) / double(m_num_points)) );
            interp_y[idx] =
                m_center[1] + ( 0.001* m_L * sin(2.0*3.1416* double(idx) / double(m_num_points)) );
            interp_z[idx] =
                m_center[2];
        }
        ...
        if (first_step)
        {
            output_file.write_header_line({"circulation values"});
        }
