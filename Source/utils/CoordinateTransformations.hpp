...
// Convert a Tensor (with three lower indices) in cylindrical coords to cartesian
// coords
template <class data_t>
static Tensor<2, Tensor<1, data_t>> 
cylindrical_to_cartesian_LLL(const Tensor<2, Tensor<1, data_t>> &cylindrical_g, const data_t x,
                          const double y, const double z)
{
    Tensor<2, Tensor<1, data_t>> cartesian_g;
    ...
    FOR(i, j, k)
    {
        ...
        FOR(w, n, l)
        {
            cartesian_g[i][j][k] += cylindrical_g[w][n][l] * jac[w][i] * jac[n][j] * jac[l][k];
        }
    }
    return cartesian_g;
}
...
// Convert a Tensor (with 1 lower indice and 1 upper indice) in cylindrical coords to cartesian
// coords
template <class data_t>
static Tensor<2, data_t> 
cylindrical_to_cartesian_UL(const Tensor<2, data_t> &cylindrical_g, const data_t x,
                          const double y, const double z)
{
    Tensor<2, data_t> cartesian_g;
    ...
    FOR(i, j)
    {
        ...
        FOR(k, l)
        {
            cartesian_g[i][j] += cylindrical_g[k][l] * inv_jac[k][i] * jac[l][j];
        }
    }
    return cartesian_g;
}
