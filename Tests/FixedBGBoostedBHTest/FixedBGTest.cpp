...
int main()
{
    ...
    for (int ires = 0; ires < num_resolutions; ires++)
    {
        ...
        BoostedIsotropicBHFixedBG boosted_bh(bg_params, dx);
        // BoostedKerrSchildBHFixedBG boosted_bh(bg_params, dx);
        BoxLoops::loop(
            AssignFixedBGtoBSSNVars<BoostedIsotropicBHFixedBG>(
                boosted_bh, dx,
                // BoxLoops::loop(AssignFixedBGtoBSSNVars<BoostedKerrSchildBHFixedBG>(boosted_bh,
                // dx,
                center_vector),
        ...
        BoxLoops::loop(
            MatterCCZ4RHS<ScalarField<Potential>>(
        ...
        // values
        BoxLoops::loop(ExcisionTest<FixedBGScalarField<Potential>,
                                    BoostedIsotropicBHFixedBG>(
